...
We first evaluate our channel selection component and show that it is able to respond to changes in the spectrum database. 
A database client has to comply with ETSI specifications~\cite{etsi_tvws} which mandate that transmissions have to stop at most one minute after the channel ceases to be available. In general, the granularity of channel availability is expected to be in hours and days~\cite{Rice_af}, as the channel is allocated to the incumbents such as wireless microphones for special events. We confirm this by inspecting the content of the database during the last year. 

We measure the time it takes for our system to vacate and reacquire a channel, following a change in a database. The experiment is illustrated in Figure~\ref{fig:paws}. Our system has only one channel available, used at the beginning of the experiment. The AP radio is active and the client is generating traffic. 
...
\begin{figure}[htb!]
  ...
  \vskip -6pt
  \caption{Spectrum database interaction experiment}
  \label{fig:paws}
  \vskip -6pt
\end{figure}

At 57sec we remove the channel from the database for 5 min, as illustrated in the top figure. We see that 2sec after the channel has been removed, the radio is off. The client stops transmitting at the same time. This is well below the 60sec ETSI requirement. 

Once the channel is reintroduced, the interference client reboots the AP to restart the radio. This takes 1min 36sec. Our current interface with the AP requires an AP reboot after any radio parameter changes, and the AP OS reboot accounts for most of this time. We note that this process can be shortened using more advanced AP control interfaces such as TR-069~\cite{tr69}. 

Once the AP is on, it takes another 56s for a client to connect to the access point and resume traffic. This is because the client has to perform cell search on various frequencies in multiple LTE bands. This time can be further reduced by disabling unused LTE bands, which we are unable to do on our existing clients. 
...
and only reports wide-band CQI over the entire 5 MHz channel. Thus, we only implemented wideband CQI reporting for this experiment. 
